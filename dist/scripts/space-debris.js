...
var _inputConfig = require('./src/input-config');

Object.keys(_inputConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _inputConfig[key];
    }
  });
});
...
},{"./src/camera-config":98,"./src/camera-folder-config":99,"./src/input-config":100,"./src/physics-config":101,"./src/physics-folder-config":102}],98:[function(require,module,exports){
'use strict';
...
},{"gamex":1}],99:[function(require,module,exports){
'use strict';
...
},{"./camera-config":98}],100:[function(require,module,exports){
'use strict';
...
inputConfig.allKeys = [inputConfig.shipShootTorpedoKey, inputConfig.shipForwardThrusterKey, inputConfig.shipStabilizerKey, inputConfig.shipPitchThrusterUpKey, inputConfig.shipPitchThrusterDownKey, inputConfig.shipYawThrusterLeftKey, inputConfig.shipYawThrusterRightKey].concat(_toConsumableArray(inputConfig.pauseKeys));
...
},{"./physics-config":101}],103:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.ChunkController = undefined;
...
var ChunkController = function () {
   ...
   * of all chunk events.
   */
  function ChunkController(sceneCenter, chunkSideLength, chunkCountOnASide, chunkListeners) {
    _classCallCheck(this, ChunkController);

    this._chunkSideLength = chunkSideLength;
    this._chunkHalfSideLength = chunkSideLength / 2;
    // Ensure this is an even number.
    this._chunkCountOnASide = chunkCountOnASide % 2 === 0 ? chunkCountOnASide : chunkCountOnASide + 1;
  ...
  _createClass(ChunkController, [{
    key: 'reset',
    value: function reset(sceneCenter) {
      this._setChunkCoordinatePositionFromScenePosition(sceneCenter);
      this._destroyAllChunks();
      this._createAllChunks();
      this._chunkListeners.forEach(function (listener) {
        return listener.onAllChunksUpdated();
      });
    }
    ...
    key: 'update',
    value: function update(sceneCenter) {
      // Convert the scene center to chunk coordinates.
      var centerInChunkCoords = this._getChunkCoordinatePositionFromScenePosition(sceneCenter);

      // Has the scene center moved to a new chunk?
      if (!(0, _gamex.areVec3sEqual)(this._prevRenderSceneCenter, centerInChunkCoords)) {
    ...
    key: '_handleSceneMovedToNewChunk',
    value: function _handleSceneMovedToNewChunk(centerInSceneCoords, centerInChunkCoords) {
      ...
      var isDiffGreaterThanCountOnASide = diff[0] > this._chunkCountOnASide || diff[1] > this._chunkCountOnASide || diff[2] > this._chunkCountOnASide;
      ...
      var positionString = (0, _gamex.vec3ToString)(centerInChunkCoords);
    ...
    key: '_updateChunksForNewCenter',
    value: function _updateChunksForNewCenter(centerDiff) {
      var xDiff = centerDiff[0];
      var yDiff = centerDiff[1];
      var zDiff = centerDiff[2];

      var chunkHalfCountOnASide = this._chunkCountOnASide / 2;

      var lowestXCoordinateToDelete = this._prevRenderSceneCenter[0] - chunkHalfCountOnASide;
      var lowestYCoordinateToDelete = this._prevRenderSceneCenter[1] - chunkHalfCountOnASide;
      var lowestZCoordinateToDelete = this._prevRenderSceneCenter[2] - chunkHalfCountOnASide;

      var lowestXCoordinateToAdd = this._prevRenderSceneCenter[0] + chunkHalfCountOnASide;
      var lowestYCoordinateToAdd = this._prevRenderSceneCenter[1] + chunkHalfCountOnASide;
      var lowestZCoordinateToAdd = this._prevRenderSceneCenter[2] + chunkHalfCountOnASide;
      ...
      for (var x = 0; x < xDiff; x++) {
        for (var y = 0; y < this._chunkCountOnASide; y++) {
          for (var z = 0; z < this._chunkCountOnASide; z++) {
            var chunk = this._getChunkFromPosition(x + lowestXCoordinateToDelete, y + lowestYCoordinateToDelete, z + lowestZCoordinateToDelete);
            if (chunk) {
      ...
      // corner).
      for (var _x = xDiff; _x < this._chunkCountOnASide; _x++) {
        for (var _y = 0; _y < yDiff; _y++) {
          for (var _z = 0; _z < this._chunkCountOnASide; _z++) {
            var _chunk = this._getChunkFromPosition(_x + lowestXCoordinateToDelete, _y + lowestYCoordinateToDelete, _z + lowestZCoordinateToDelete);
            if (_chunk) {
              this._destroyChunk(_chunk);
              this._chunks.delete(_chunk.id);
            }
          }
        }
      }
      ...
      for (var _x2 = xDiff; _x2 < this._chunkCountOnASide; _x2++) {
        for (var _y2 = yDiff; _y2 < this._chunkCountOnASide; _y2++) {
          for (var _z2 = 0; _z2 < zDiff; _z2++) {
            var _chunk2 = this._getChunkFromPosition(_x2 + lowestXCoordinateToDelete, _y2 + lowestYCoordinateToDelete, _z2 + lowestZCoordinateToDelete);
            if (_chunk2) {
              this._destroyChunk(_chunk2);
              this._chunks.delete(_chunk2.id);
            }
          }
        }
      }
      ...
      for (var _x3 = 0; _x3 < xDiff; _x3++) {
        for (var _y3 = 0; _y3 < this._chunkCountOnASide; _y3++) {
          for (var _z3 = 0; _z3 < this._chunkCountOnASide; _z3++) {
            vec3.set(chunkPosition, _x3 + lowestXCoordinateToAdd, _y3 + lowestYCoordinateToDelete, _z3 + lowestZCoordinateToDelete);
            var _chunk3 = this._createChunk(chunkPosition);
            this._chunks.set(_chunk3.id, _chunk3);
          }
        }
      }

      // Add new chunks to the delta-y face (and to the y-z corner, but not to the x-y corner).
      for (var _x4 = xDiff; _x4 < this._chunkCountOnASide; _x4++) {
        for (var _y4 = 0; _y4 < yDiff; _y4++) {
          for (var _z4 = 0; _z4 < this._chunkCountOnASide; _z4++) {
            vec3.set(chunkPosition, _x4 + lowestXCoordinateToDelete, _y4 + lowestYCoordinateToAdd, _z4 + lowestZCoordinateToDelete);
            var _chunk4 = this._createChunk(chunkPosition);
            this._chunks.set(_chunk4.id, _chunk4);
          }
        }
      }

      // Add new chunks to the delta-z face (but not to the x-z or y-z corners).
      for (var _x5 = xDiff; _x5 < this._chunkCountOnASide; _x5++) {
        for (var _y5 = yDiff; _y5 < this._chunkCountOnASide; _y5++) {
          for (var _z5 = 0; _z5 < zDiff; _z5++) {
            vec3.set(chunkPosition, _x5 + lowestXCoordinateToDelete, _y5 + lowestYCoordinateToDelete, _z5 + lowestZCoordinateToAdd);
            var _chunk5 = this._createChunk(chunkPosition);
            this._chunks.set(_chunk5.id, _chunk5);
          }
        }
      }
    }
    ...
    key: '_destroyAllChunks',
    value: function _destroyAllChunks() {
      var _this = this;

      this._chunks.forEach(function (chunk) {
        return _this._destroyChunk(chunk);
      });
      this._chunks.clear();
    }
    ...
    key: '_createAllChunks',
    value: function _createAllChunks() {
      var chunkPosition = vec3.create();

      var lowestChuckOffsetFromCenter = -this._chunkCountOnASide / 2;
      var lowestChunkXCoordinate = this._prevRenderSceneCenter[0] + lowestChuckOffsetFromCenter;
      var lowestChunkYCoordinate = this._prevRenderSceneCenter[1] + lowestChuckOffsetFromCenter;
      var lowestChunkZCoordinate = this._prevRenderSceneCenter[2] + lowestChuckOffsetFromCenter;

      for (var x = 0; x < this._chunkCountOnASide; x++) {
        for (var y = 0; y < this._chunkCountOnASide; y++) {
          for (var z = 0; z < this._chunkCountOnASide; z++) {
            vec3.set(chunkPosition, lowestChunkXCoordinate + x, lowestChunkYCoordinate + y, lowestChunkZCoordinate + z);
            var chunk = this._createChunk(chunkPosition);
    ...
    key: '_createChunk',
    value: function _createChunk(chunkPosition) {
      var centerInSceneCoords = this._getSceneCoordinateChunkCenterPositionFromChunkPosition(chunkPosition);
      var chunk = {
        id: ChunkController._getIdFromChunkPosition(chunkPosition[0], chunkPosition[1], chunkPosition[2]),
        bounds: _gamex.Aabb.createAsUniformAroundCenter(centerInSceneCoords, this._chunkHalfSideLength)
      };
      this._chunkListeners.forEach(function (listener) {
        return listener.onChunkCreated(chunk);
      });
      return chunk;
    }
    ...
    key: '_destroyChunk',
    value: function _destroyChunk(chunk) {
      this._chunkListeners.forEach(function (listener) {
        return listener.onChunkDestroyed(chunk);
      });
    }
    ...
    key: '_getSceneCoordinateChunkCenterPositionFromChunkPosition',
    value: function _getSceneCoordinateChunkCenterPositionFromChunkPosition(chunkPosition) {
      return vec3.fromValues(Math.floor(chunkPosition[0] * this._chunkSideLength + this._chunkHalfSideLength), Math.floor(chunkPosition[1] * this._chunkSideLength + this._chunkHalfSideLength), Math.floor(chunkPosition[2] * this._chunkSideLength + this._chunkHalfSideLength));
    }
    ...
    key: '_getChunkCoordinatePositionFromScenePosition',
    value: function _getChunkCoordinatePositionFromScenePosition(sceneCenter) {
      return vec3.fromValues(Math.floor(sceneCenter[0] / this._chunkSideLength), Math.floor(sceneCenter[1] / this._chunkSideLength), Math.floor(sceneCenter[2] / this._chunkSideLength));
    }
    ...
    key: '_setChunkCoordinatePositionFromScenePosition',
    value: function _setChunkCoordinatePositionFromScenePosition(sceneCenter) {
      vec3.copy(this._prevRenderSceneCenter, this._getChunkCoordinatePositionFromScenePosition(sceneCenter));
    }
    ...
    key: '_getChunkFromPosition',
    value: function _getChunkFromPosition(x, y, z) {
    ...
    key: '_getIdFromChunkPosition',
    value: function _getIdFromChunkPosition(x, y, z) {
      return x + ',' + y + ',' + z;
    }
  }]);
...
exports.ChunkController = ChunkController;
...
},{"gamex":1}],104:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.HealthController = undefined;
...
var HealthController = function () {
   ...
   */
  function HealthController(healthMeter, onHealthDepleted) {
    _classCallCheck(this, HealthController);
  ...
  _createClass(HealthController, [{
    key: 'decrement',
    value: function decrement() {
      this._healthCount--;
      var ratio = this._healthCount * 1.0 / INITIAL_HEALTH_COUNT;
    ...
    key: '_updateHealthMeter',
    value: function _updateHealthMeter(ratio) {
      this._healthMeter.style.width = ratio * this._capacityWidth + 'px';
      this._healthMeter.style.backgroundColor = (0, _gamex.createHslColorString)({
...
},{"gamex":1}],105:[function(require,module,exports){
'use strict';
...
var POINTS_FOR_ASTEROID = 100;

var ScoreController = function () {
  /**
   * @param {HTMLElement} scorePanel
   */
  function ScoreController(scorePanel) {
  ...
  _createClass(ScoreController, [{
    key: 'onAsteroidShot',
    value: function onAsteroidShot() {
      this._score += POINTS_FOR_ASTEROID;
      this._updateScorePanel();
    }
  }, {
    key: '_updateScorePanel',
    value: function _updateScorePanel() {
     ...
     */
    value: function updateBestScore() {
...
exports.SpaceDebrisController = undefined;
...
var _healthController = require('./health-controller');

var _scoreController = require('./score-controller');
 ...
 * This top-level Controller class initializes and runs the rest of the app.
 */
var SpaceDebrisController = function (_GameController) {
  _inherits(SpaceDebrisController, _GameController);

  function SpaceDebrisController() {
    _classCallCheck(this, SpaceDebrisController);

    var _this = _possibleConstructorReturn(this, (SpaceDebrisController.__proto__ || Object.getPrototypeOf(SpaceDebrisController)).call(this));

    _this._pauseScreen = document.querySelector('.pause-screen');
    _this._title = document.querySelector('.title');
    _this._playButton = document.querySelector('button.play');
    _this._newRecordField = document.querySelector('.new-record');
    _this._toastPanel = document.querySelector('.toast');

    _this.healthCtrl = null;
    _this.scoreCtrl = null;

    _this._setElementListeners();
    return _this;
  }
  ...
  _createClass(SpaceDebrisController, [{
    key: 'initialize',
    value: function initialize(canvas, programConfigs, texturePaths, SceneImpl) {
      var _this2 = this;

      var healthMeter = document.querySelector('.health .current');
      var scorePanel = document.querySelector('.score');

      this.healthCtrl = new _healthController.HealthController(healthMeter, this.endGame.bind(this));
      this.scoreCtrl = new _scoreController.ScoreController(scorePanel);

      return _get(SpaceDebrisController.prototype.__proto__ || Object.getPrototypeOf(SpaceDebrisController.prototype), 'initialize', this).call(this, canvas, programConfigs, texturePaths, SceneImpl).then(function () {
        _this2._setPauseKeyListeners();
        _this2._setHideToastListeners();
      });
    }
  }, {
    key: 'run',
    value: function run() {
      _get(SpaceDebrisController.prototype.__proto__ || Object.getPrototypeOf(SpaceDebrisController.prototype), 'run', this).call(this);
    ...
    value: function endGame() {
      ...
      var isNewRecord = this.scoreCtrl.updateBestScore();
    ...
    key: 'pause',
    value: function pause() {
      _get(SpaceDebrisController.prototype.__proto__ || Object.getPrototypeOf(SpaceDebrisController.prototype), 'pause', this).call(this);
      this._updatePauseMenu();
      this._pauseScreen.style.display = 'block';
    }
  }, {
    key: 'unpause',
    value: function unpause() {
      _get(SpaceDebrisController.prototype.__proto__ || Object.getPrototypeOf(SpaceDebrisController.prototype), 'unpause', this).call(this);
    ...
    key: '_updatePauseMenu',
    value: function _updatePauseMenu() {
      var scoreField = document.querySelector('.pause-menu-score');
      var highestScoreField = document.querySelector('.pause-menu-highest-score');
      var runTimeField = document.querySelector('.pause-menu-total-run-time');

      var scoreText = this.scoreCtrl.score + ' points';
      var highestScoreText = this.scoreCtrl.bestScore + ' points';
      var runTimeText = millisecondsToString(_gamex.animator.totalRunTime);
    ...
    key: '_setPauseKeyListeners',
    value: function _setPauseKeyListeners() {
      var _this3 = this;

      _config.inputConfig.pauseKeys.forEach(function (key) {
        return _this3._inputCtrl.addKeyDownListener(key, function () {
    ...
    key: '_setHideToastListeners',
    value: function _setHideToastListeners() {
      var _this4 = this;

      var gameKeyPressCount = 0;
      var handleGameKeyPress = function handleGameKeyPress() {
        gameKeyPressCount++;
        if (gameKeyPressCount >= 2) {
          _this4._toastPanel.style.opacity = '0';

          // Remove listeners.
          _config.inputConfig.allKeys.forEach(function (key) {
            return _this4._inputCtrl.removeKeyDownListener(key, handleGameKeyPress);
          });
        }
      };
      ...
      // Add listeners.
      _config.inputConfig.allKeys.forEach(function (key) {
        return _this4._inputCtrl.addKeyDownListener(key, handleGameKeyPress);
      });
    }
  }, {
    key: '_setElementListeners',
    value: function _setElementListeners() {
      var _this5 = this;

      this._playButton.addEventListener('click', function () {
        return _this5.unpause();
      }, false);
      this._pauseScreen.addEventListener('click', function (event) {
        return _this5._onScreenClicked(event);
      }, false);
    }
  }, {
    key: '_onScreenClicked',
    value: function _onScreenClicked(event) {
...
}(_gamex.GameController);

function millisecondsToString(totalMilliseconds) {
  var milliseconds = totalMilliseconds % 1000;
  var tmp = (totalMilliseconds - milliseconds) / 1000;
  var seconds = tmp % 60;
  tmp = (tmp - seconds) / 60;
  var minutes = tmp % 60;
...
exports.SpaceDebrisController = SpaceDebrisController;

},{"../config":97,"./health-controller":104,"./score-controller":105,"gamex":1}],107:[function(require,module,exports){
'use strict';
...
var _spaceDebrisController = require('./core/space-debris-controller');

var _scene = require('./model-controllers/scene');
...
function _initApp() {
  ...
  var controller = new _spaceDebrisController.SpaceDebrisController();
...
},{"./config":97,"./core/space-debris-controller":106,"./model-controllers/scene":113,"./programs":141,"gamex":1}],108:[function(require,module,exports){
'use strict';
...
var _asteroidController = require('./src/asteroid-controller');

Object.keys(_asteroidController).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _asteroidController[key];
    }
  });
});

var _asteroidShapeConfig = require('./src/asteroid-shape-config');

Object.keys(_asteroidShapeConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _asteroidShapeConfig[key];
    }
  });
});

var _asteroidsConfig = require('./src/asteroids-config');

Object.keys(_asteroidsConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _asteroidsConfig[key];
    }
  });
});

var _asteroidsController = require('./src/asteroids-controller');

Object.keys(_asteroidsController).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _asteroidsController[key];
    }
  });
});
...
_gamex.renderableShapeFactory.registerRenderableShapeFactory(_asteroidShapeConfig.asteroidRenderableShapeFactory);

},{"./src/asteroid-controller":109,"./src/asteroid-shape-config":110,"./src/asteroids-config":111,"./src/asteroids-controller":112,"gamex":1}],109:[function(require,module,exports){
'use strict';
...
exports.AsteroidController = undefined;
...
var _asteroidsConfig = require('./asteroids-config');
 ...
 */
var AsteroidController = function (_CollidablePhysicsMod) {
  _inherits(AsteroidController, _CollidablePhysicsMod);
   ...
   */
  function AsteroidController(modelControllerParams, dynamicsParams, scale, divisionsCount, scoreCtrl, onDestroyedCallback) {
    _classCallCheck(this, AsteroidController);

    modelControllerParams.programWrapperId = _asteroidsConfig.asteroidsConfig.shaderProgram;
    modelControllerParams.texturePath = _asteroidsConfig.asteroidsConfig.texturePath;
    ...
    var _this = _possibleConstructorReturn(this, (AsteroidController.__proto__ || Object.getPrototypeOf(AsteroidController)).call(this, modelControllerParams, dynamicsParams, shapeParams, forceAppliers));

    _this._scoreCtrl = scoreCtrl;
    _this._onDestroyedCallback = onDestroyedCallback;
    return _this;
  }
  ...
  _createClass(AsteroidController, [{
    key: 'handleCollision',
    value: function handleCollision(collision) {
      var collidableType = (0, _gamex.getOtherControllerFromCollision)(collision, this).constructor.name;
...
},{"./asteroids-config":111,"gamex":1}],110:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.asteroidRenderableShapeFactory = undefined;
...
var asteroidRenderableShapeFactory = {
  shapeId: 'ASTEROID',
   ...
   */
  getRenderableShape: function getRenderableShape(params) {
    var copyParams = (0, _gamex.deepCopy)(params);
    copyParams.shapeId = 'ICOSPHERE';
    var icosphereRenderableShape = _gamex.renderableShapeFactory.getRenderableShape(copyParams);
    ...
    icosphereRenderableShape.vertexNormals = [].concat(_toConsumableArray(icosphereRenderableShape.vertexNormals));
   ...
   */
  getCacheId: function getCacheId(params) {
    return (0, _gamex.getCacheKey)(params);
  }
};
...
function _addRandomOffsetsToPositions(vertexPositions) {
  ...
  var orthogonalOffsetVector = vec3.create();

  var i = void 0;
  var count = void 0;
  var orthogonalOffset = void 0;
  ...
  for (i = 0, count = vertexPositions.length; i < count; i += 3) {
    ...
    // Orthogonal offset.
    orthogonalOffset = (0, _gamex.randomFloatInRange)(_asteroidsConfig.asteroidsConfig.minVertexOrthogonalDeviation, _asteroidsConfig.asteroidsConfig.maxVertexOrthogonalDeviation);
...
exports.asteroidRenderableShapeFactory = asteroidRenderableShapeFactory;

},{"./asteroids-config":111,"gamex":1}],111:[function(require,module,exports){
'use strict';
...
var _oneRevolutionPerSecond = Math.PI / 500;
...
function _updateRemovalSquaredDistance() {
  asteroidsConfig._removalSquaredDistance = asteroidsConfig.removalDistance * asteroidsConfig.removalDistance;
}

_updateRemovalSquaredDistance();

var asteroidsFolderConfig = {
  label: 'Asteroids',
...
exports.asteroidsConfig = asteroidsConfig;
exports.asteroidsFolderConfig = asteroidsFolderConfig;
...
exports.AsteroidsController = undefined;
...
var _asteroidController = require('./asteroid-controller');
 ...
 */
var AsteroidsController = function (_ModelGroupController) {
  _inherits(AsteroidsController, _ModelGroupController);
   ...
   */
  function AsteroidsController(modelControllerParams, scoreCtrl, getShipPosition, getShipVelocity) {
    _classCallCheck(this, AsteroidsController);

    var _this = _possibleConstructorReturn(this, (AsteroidsController.__proto__ || Object.getPrototypeOf(AsteroidsController)).call(this, modelControllerParams));

    _this._scoreCtrl = scoreCtrl;
    _this._getShipPosition = getShipPosition;
    _this._getShipVelocity = getShipVelocity;
    _this._throttledRemoveDistantAsteroids = (0, _gamex.throttle)(function () {
      return _this._removeDistantAsteroids();
    }, _asteroidsConfig.asteroidsConfig._removalCheckThrottleDelay);
    _this._lastAsteroidSpawnTime = null;
    return _this;
  }

  _createClass(AsteroidsController, [{
    key: 'reset',
    value: function reset() {
      this.clearModelControllers();
      this._createInitialAsteroids();
      this._lastAsteroidSpawnTime = _gamex.animator.currentTime;
    }
    ...
    key: '_handleInFlightAsteroidCreation',
    value: function _handleInFlightAsteroidCreation(currentTime) {
      var _this2 = this;

      var elapsedTime = currentTime - this._lastAsteroidSpawnTime;
      var spawnPeriod = _asteroidsConfig.asteroidsConfig._spawnPeriodShipSpeedMultiplier / vec3.length(this._getShipVelocity());
      if (elapsedTime > spawnPeriod) {
        this._lastAsteroidSpawnTime = currentTime;
        this._createInFlightAsteroid().then(function (asteroid) {
          return _this2._ensureAsteroidIsNotColliding(asteroid);
        });
      }
    }
    ...
    key: '_createInitialAsteroids',
    value: function _createInitialAsteroids() {
      var _this3 = this;

      var promises = [];
      for (var i = 0; i < _asteroidsConfig.asteroidsConfig.initialAsteroidCount; i++) {
        promises.push(this._createInitialAsteroid());
      }
      Promise.all(promises).then(function () {
        return _this3._ensureNoAsteroidsCollide();
      });
    }
    ...
    key: '_createInitialAsteroid',
    value: function _createInitialAsteroid() {
      var position = this._createRandomInitialAsteroidPosition();
      var velocity = this._createRandomAsteroidVelocity(position, _asteroidsConfig.asteroidsConfig._initialMinSpeed, _asteroidsConfig.asteroidsConfig._initialMaxSpeed);
    ...
    key: '_createInFlightAsteroid',
    value: function _createInFlightAsteroid() {
      // Make sure we don't create too many asteroids.
      if (this._modelCtrls.length >= _asteroidsConfig.asteroidsConfig.maxAsteroidCount) return Promise.reject();

      var position = this._createRandomInFlightAsteroidPosition();
      var velocity = this._createRandomAsteroidVelocity(position, _asteroidsConfig.asteroidsConfig.minSpeed, _asteroidsConfig.asteroidsConfig.maxSpeed);
      return this._createAsteroid(position, velocity);
    }
    ...
    key: '_createAsteroid',
    value: function _createAsteroid(position, velocity) {
      var _this4 = this;

      var scale = this._createRandomAsteroidScale();
      var rotationRate = this._createRandomAsteroidRotationRate();
      var divisionsCount = this._createRandomAsteroidDivisionsCount();
      ...
      var dynamicsParams = _calculateAsteroidDynamicsConfig(_asteroidsConfig.asteroidsConfig.density, position, velocity, rotationRate, scale);

      var asteroid = new _asteroidController.AsteroidController(modelControllerParams, dynamicsParams, scale, divisionsCount, this._scoreCtrl, function (asteroid) {
        return _this4._onModelControllerDestroyed(asteroid);
      });
    ...
    key: '_createRandomInitialAsteroidPosition',
    value: function _createRandomInitialAsteroidPosition() {
      var distanceFromShip = (0, _gamex.randomFloatInRange)(_asteroidsConfig.asteroidsConfig._initialAsteroidMinDistanceFromShip, _asteroidsConfig.asteroidsConfig._initialAsteroidMaxDistanceFromShip);
      var shipPosition = this._getShipPosition();

      // Direction from ship.
      var asteroidPosition = (0, _gamex.createRandomVec3)();
    ...
    key: '_createRandomInFlightAsteroidPosition',
    value: function _createRandomInFlightAsteroidPosition() {
      // Calculate some parameters for the position within a virtual cone.
      var r = (0, _gamex.randomFloatInRange)(0, 1);
      // Weight the angle to be further toward the outside of the the virtual cone.
      var ratio = 1 - r * r;
      var lateralAngle = ratio * _asteroidsConfig.asteroidsConfig.positionMaxAngleFromShipTrajectory;
      r = (0, _gamex.randomFloatInRange)(0, 1);
      // Weight the distance to favor being further away.
      ratio = 1 - r * r;
      var distance = _asteroidsConfig.asteroidsConfig.minDistanceFromShip + (_asteroidsConfig.asteroidsConfig.maxDistanceFromShip - _asteroidsConfig.asteroidsConfig.minDistanceFromShip) * ratio;
      var axialAngle = (0, _gamex.randomFloatInRange)(0, _gamex.TWO_PI);
      var origin = vec3.create();

      // Start with a position at the given distance.
      var position = vec3.fromValues(0, 0, -distance);
      ...
      var shipVelocity = this._getShipVelocity();
      vec3.normalize(shipVelocity, shipVelocity);
      var rotationX = Math.asin(shipVelocity[1]);
      var rotationY = -Math.atan2(shipVelocity[0], -shipVelocity[2]);
    ...
    key: '_ensureNoAsteroidsCollide',
    value: function _ensureNoAsteroidsCollide() {
      // Loop in reverse order so we only move the asteroids that are added later.
      for (var i = this._modelCtrls.length - 1; i >= 0; i--) {
    ...
    key: '_ensureAsteroidIsNotColliding',
    value: function _ensureAsteroidIsNotColliding(asteroid) {
      var collidable = asteroid.physicsJob.collidable;
      var radius = (0, _gamex.getMaxVec3Dimension)(asteroid.scale);
      var asteroidPosition = asteroid.position;

      var shipToAsteroidOffset = vec3.create();
    ...
    key: '_createRandomAsteroidVelocity',
    value: function _createRandomAsteroidVelocity(asteroidPosition, minSpeed, maxSpeed) {
      var shipPosition = this._getShipPosition();
      var shipVelocity = this._getShipVelocity();

      // Calculate a random speed for the asteroid.
      var asteroidSpeed = (0, _gamex.randomFloatInRange)(minSpeed, maxSpeed);
      ...
      var interceptVelocity = vec3.create();
      (0, _util.calculateInterceptVelocity)(interceptVelocity, asteroidPosition, asteroidSpeed, shipPosition, shipVelocity);
      ...
      (0, _gamex.addRandomRotationToVector)(interceptVelocity, _asteroidsConfig.asteroidsConfig.minInterceptVelocityDeviationAngle, _asteroidsConfig.asteroidsConfig.maxInterceptVelocityDeviationAngle);
    ...
    key: '_createRandomAsteroidScale',
    value: function _createRandomAsteroidScale() {
      var value = (0, _gamex.randomFloatInRange)(_asteroidsConfig.asteroidsConfig.minScale, _asteroidsConfig.asteroidsConfig.maxScale);
    ...
    key: '_createRandomAsteroidRotationRate',
    value: function _createRandomAsteroidRotationRate() {
      return (0, _gamex.randomFloatInRange)(_asteroidsConfig.asteroidsConfig.minRotationRate, _asteroidsConfig.asteroidsConfig.maxRotationRate);
    }
    ...
    key: '_createRandomAsteroidDivisionsCount',
    value: function _createRandomAsteroidDivisionsCount() {
      return (0, _gamex.randomIntInRange)(_asteroidsConfig.asteroidsConfig.minDivisionsCount, _asteroidsConfig.asteroidsConfig.maxDivisionsCount);
    }
  }, {
    key: '_removeDistantAsteroids',
    value: function _removeDistantAsteroids() {
      var _this5 = this;

      this._removeDistantModelControllers(this._getShipPosition(), _asteroidsConfig.asteroidsConfig._removalSquaredDistance);

      // Make sure we don't have too few asteroids.
      var asteroidDeficit = _asteroidsConfig.asteroidsConfig.minAsteroidCount - this._modelCtrls.length;
      if (asteroidDeficit > 0) {
        var promises = [];
        for (var i = 0; i < asteroidDeficit; i++) {
          promises.push(this._createInFlightAsteroid());
        }
        Promise.all(promises).then(function () {
          return _this5._ensureNoAsteroidsCollide();
        });
      }
    }
...
function _calculateAsteroidDynamicsConfig(density, startPosition, velocity, rotationRate, scale) {
  var radius = (scale[0] + scale[1] + scale[2]) / 3;
  var mass = density * 4 / 3 * Math.PI * radius * radius * radius;

  var momentum = vec3.create();
  vec3.scale(momentum, velocity, mass);
  var startOrientation = quat.create();
  var angularMomentum = _calculateAngularMomentumFromRotationRate(rotationRate, radius, mass);
...
function _calculateAngularMomentumFromRotationRate(rotationRate, radius, mass) {
  var inertiaTensor = (0, _gamex.createSphereInertiaTensor)(radius, mass);
  var angularMomentum = (0, _gamex.createRandomVec3)(rotationRate);
  vec3.transformMat3(angularMomentum, angularMomentum, inertiaTensor);
  return angularMomentum;
}

exports.AsteroidsController = AsteroidsController;

},{"../../../util":143,"./asteroid-controller":109,"./asteroids-config":111,"gamex":1}],113:[function(require,module,exports){
'use strict';
...
var _sceneConfig = require('./src/scene-config');

Object.keys(_sceneConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _sceneConfig[key];
    }
  });
});

var _sceneImpl = require('./src/scene-impl');

Object.keys(_sceneImpl).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _sceneImpl[key];
    }
  });
});

var _shipDestroyedCameraTarget = require('./src/ship-destroyed-camera-target');

Object.keys(_shipDestroyedCameraTarget).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _shipDestroyedCameraTarget[key];
    }
  });
});

},{"./src/scene-config":114,"./src/scene-impl":115,"./src/ship-destroyed-camera-target":116}],114:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.sceneConfig = undefined;
...
var _chunkController = require('../../../core/chunk-controller');

var _torpedo = require('../../torpedo');
...
var _shipDestroyedCameraTarget = require('./ship-destroyed-camera-target');
...
var SceneImpl = function (_GameScene) {
  ...
  function SceneImpl(modelControllerParams, gameCtrl, inputCtrl) {
    ...
    _this._asteroidsCtrl = null;
    _this._torpedoesCtrl = null;
    _this._shipCtrl = null;
    _this._starsCtrl = null;
    _this._ufosCtrl = null;
    _this._chunkCtrl = null;
    // This is either _shipCtrl or _shipDestroyedCameraTarget depending on whether the game is over.
    _this._shipProxy = null;
    _this._shipDestroyedCameraTarget = null;
    _this._camera = null;
    _this._chunkListeners = [];

    _this._createConfigController();
    _this._createChunkController();
    _this._isReadyPromise = _this._createModelControllers(inputCtrl);
    _this._createCamera();
    return _this;
  }
  ...
  _createClass(SceneImpl, [{
    key: 'reset',
    value: function reset() {
      _get(SceneImpl.prototype.__proto__ || Object.getPrototypeOf(SceneImpl.prototype), 'reset', this).call(this);
    ...
    key: 'updateChildren',
    value: function updateChildren(currentTime, deltaTime) {
      _get(SceneImpl.prototype.__proto__ || Object.getPrototypeOf(SceneImpl.prototype), 'updateChildren', this).call(this, currentTime, deltaTime);
    ...
    value: function destroyShip() {
      ...
      this._shipDestroyedCameraTarget = new _shipDestroyedCameraTarget.ShipDestroyedCameraTarget(this._shipCtrl);
      this._startModelController(this._shipDestroyedCameraTarget).then(function () {
        _this2._shipProxy = _this2._shipDestroyedCameraTarget;
        _this2._createCamera();
        _this2._onModelControllerDestroyed(_this2._shipCtrl);
      });
    ...
    value: function _createModelControllers(inputCtrl) {
      var _this3 = this;

      var getWorldTransform = function getWorldTransform() {
        return _this3.worldTransform;
      };
      var getCameraPosition = function getCameraPosition() {
        return _this3._camera.position;
      };
      var getShipPosition = function getShipPosition() {
        return _this3._shipProxy.position;
      };
      var getShipVelocity = function getShipVelocity() {
        return _this3._shipProxy.velocity;
      };
      var modelControllerParams = {
        gl: this._gl,
        getViewMatrix: this._getViewMatrix,
      ...
      this._asteroidsCtrl = new _asteroid.AsteroidsController((0, _gamex.shallowCopy)(modelControllerParams), this._gameCtrl.scoreCtrl, getShipPosition, getShipVelocity);
      this._torpedoesCtrl = new _torpedo.TorpedoesController((0, _gamex.shallowCopy)(modelControllerParams), getShipPosition);
      this._shipCtrl = new _ship.ShipController((0, _gamex.shallowCopy)(modelControllerParams), inputCtrl, this._gameCtrl.healthCtrl, this._torpedoesCtrl);
      this._shipProxy = this._shipCtrl;
      this._starsCtrl = new _star.StarsController((0, _gamex.shallowCopy)(modelControllerParams), getCameraPosition);
      // TODO
    ...
    key: '_startModelControllers',
    value: function _startModelControllers() {
      var _this4 = this;

      var modelCtrls = [this._asteroidsCtrl, this._torpedoesCtrl, this._shipCtrl, this._starsCtrl];

      return Promise.all(modelCtrls.map(function (modelCtrl) {
        return _this4._startModelController(modelCtrl);
      })).then(function () {
        _this4._chunkListeners.push(_this4._starsCtrl);
        _this4._chunkCtrl.reset(_this4.centerOfVolume);
      });
    }
  }, {
    key: '_createChunkController',
    value: function _createChunkController() {
      this._chunkCtrl = new _chunkController.ChunkController(this.centerOfVolume, _sceneConfig.sceneConfig.chunkSideLength, _sceneConfig.sceneConfig.chunkCountOnASide, this._chunkListeners);
    }
  }, {
    key: '_createCamera',
    value: function _createCamera() {
      ...
      var cameraType = _config.cameraConfig.cameraType instanceof Object ? _config.cameraConfig.cameraType.start : _config.cameraConfig.cameraType;
      var cameraClass = _config.cameraTypeMap[cameraType];
      var cameraTarget = this._createCameraTarget();

      var cameraTypeToFactory = {
        'firstPerson': function firstPerson() {
          return new cameraClass(cameraTarget, _config.firstPersonCameraConfig, _config.cameraConfig, _this5._camera);
        },
        'thirdPersonSpring': function thirdPersonSpring() {
          return new cameraClass(cameraTarget, _config.followCameraConfig, _config.cameraConfig, _this5._camera);
        },
        'thirdPersonFixed': function thirdPersonFixed() {
          return new cameraClass(cameraTarget, _config.followCameraConfig, _config.cameraConfig, _this5._camera);
        },
        'fixed': function fixed() {
          return new cameraClass(_config.fixedCameraConfig, _config.cameraConfig, _this5._camera);
        }
      };
    ...
    key: '_createCameraTarget',
    value: function _createCameraTarget() {
      var shipProxy = this._shipProxy;
    ...
    value: function _createConfigController() {
      ...
      _gamex.configController.createFolder(_asteroid.asteroidsFolderConfig, null, {
        'texturePath': function texturePath() {
          return _this6._updateAsteroidsTexture();
        },
        'shaderProgram': function shaderProgram() {
          return _this6._updateAsteroidsProgramWrapper();
        }
      });
      _gamex.configController.createFolder(_torpedo.torpedoesFolderConfig, null, {
        'shaderProgram': function shaderProgram() {
          return _this6._updateTorpedoesProgramWrapper();
        }
      });
      _gamex.configController.createFolder(_ship.shipFolderConfig, null, {
        'texturePath': function texturePath() {
          return _this6._updateShipTexture();
        },
        'shaderProgram': function shaderProgram() {
    ...
    value: function _updateAsteroidsTexture() {
      if (this._asteroidsCtrl) {
        this._asteroidsCtrl.updateTexture(_asteroid.asteroidsConfig.texturePath);
      }
    }
  }, {
    key: '_updateShipTexture',
    value: function _updateShipTexture() {
      if (this._shipCtrl) {
        this._shipCtrl.updateTexture(_ship.shipConfig.texturePath);
      }
    }
  }, {
    key: '_updateUfosTexture',
    value: function _updateUfosTexture() {
      if (this._ufosCtrl) {
        this._ufosCtrl.updateTexture(_ufo.ufosConfig.texturePath);
      }
    }
  }, {
    key: '_updateAsteroidsProgramWrapper',
    value: function _updateAsteroidsProgramWrapper() {
      if (this._asteroidsCtrl) {
        this._asteroidsCtrl.updateProgramWrapper(_asteroid.asteroidsConfig.shaderProgram);
      }
    }
  }, {
    key: '_updateShipProgramWrapper',
    value: function _updateShipProgramWrapper() {
      if (this._shipCtrl) {
        this._shipCtrl.updateProgramWrapper(_ship.shipConfig.shaderProgram);
      }
    }
  }, {
    key: '_updateTorpedoesProgramWrapper',
    value: function _updateTorpedoesProgramWrapper() {
      if (this._torpedoesCtrl) {
        this._torpedoesCtrl.updateProgramWrapper(_torpedo.torpedoesConfig.shaderProgram);
      }
    }
  }, {
    key: '_updateUfosProgramWrapper',
    value: function _updateUfosProgramWrapper() {
      if (this._ufosCtrl) {
        this._ufosCtrl.updateProgramWrapper(_ufo.ufosConfig.shaderProgram);
      }
    }
  }]);
...
},{"../../../config":97,"../../../core/chunk-controller":103,"../../asteroid":108,"../../ship":117,"../../star":123,"../../torpedo":126,"../../ufo":130,"./scene-config":114,"./ship-destroyed-camera-target":116,"gamex":1}],116:[function(require,module,exports){
'use strict';
...
exports.ShipDestroyedCameraTarget = undefined;
 ...
 */
var ShipDestroyedCameraTarget = function (_PhysicsModelControll) {
  _inherits(ShipDestroyedCameraTarget, _PhysicsModelControll);

  /**
   * @param {ShipController} shipCtrl
   */
  function ShipDestroyedCameraTarget(shipCtrl) {
    _classCallCheck(this, ShipDestroyedCameraTarget);
    ...
    var modelCtrl = new _gamex.InvisibleModelController(modelCtrlParams);

    // Copy the ship's last state.
    var physicsJob = new _gamex.PhysicsJob();
    physicsJob.currentState = new _gamex.PhysicsState(shipCtrl.physicsJob.currentState);
    physicsJob.previousState = new _gamex.PhysicsState(shipCtrl.physicsJob.previousState);
    physicsJob.renderState = new _gamex.PhysicsState(shipCtrl.physicsJob.renderState);

    var forceAppliers = [_gamex.applyAngularDrag.bind(null, _config.physicsConfig)];

    return _possibleConstructorReturn(this, (ShipDestroyedCameraTarget.__proto__ || Object.getPrototypeOf(ShipDestroyedCameraTarget)).call(this, modelCtrl, physicsJob, undefined, forceAppliers));
  }

  return ShipDestroyedCameraTarget;
}(_gamex.PhysicsModelController);

exports.ShipDestroyedCameraTarget = ShipDestroyedCameraTarget;

},{"../../../config":97,"gamex":1}],117:[function(require,module,exports){
'use strict';
...
var _shipConfig = require('./src/ship-config');

Object.keys(_shipConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _shipConfig[key];
    }
  });
});

var _shipController = require('./src/ship-controller');

Object.keys(_shipController).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _shipController[key];
    }
  });
});

var _shipShapeConfig = require('./src/ship-shape-config');

Object.keys(_shipShapeConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _shipShapeConfig[key];
    }
  });
});
...
var _shipForwardThrusterShapeConfig = require('./src/ship-forward-thruster-shape-config');

_gamex.renderableShapeFactory.registerRenderableShapeFactory(_shipShapeConfig.shipRenderableShapeFactory);
_gamex.renderableShapeFactory.registerRenderableShapeFactory(_shipForwardThrusterShapeConfig.shipForwardThrusterRenderableShapeFactory);

},{"./src/ship-config":118,"./src/ship-controller":119,"./src/ship-forward-thruster-shape-config":121,"./src/ship-shape-config":122,"gamex":1}],118:[function(require,module,exports){
'use strict';
...
var shipFolderConfig = {
  label: 'Ship',
  config: shipConfig,
  isOpen: false
};

exports.shipConfig = shipConfig;
exports.shipFolderConfig = shipFolderConfig;
...
exports.ShipController = undefined;
...
var _shipConfig = require('./ship-config');

var _shipForwardThrusterController = require('./ship-forward-thruster-controller');
 ...
 */
var ShipController = function (_CollidablePhysicsMod) {
  _inherits(ShipController, _CollidablePhysicsMod);
   ...
   */
  function ShipController(modelControllerParams, inputCtrl, healthCtrl, torpedoesCtrl) {
    _classCallCheck(this, ShipController);

    modelControllerParams.programWrapperId = _shipConfig.shipConfig.shaderProgram;
    modelControllerParams.texturePath = _shipConfig.shipConfig.texturePath;

    var shapeParams = {
      shapeId: 'SHIP',
      collidableShapeId: 'SPHERE',
      shipWidth: _shipConfig.shipConfig.shipWidth,
      shipLength: _shipConfig.shipConfig.shipLength,
      shipDepth: _shipConfig.shipConfig.shipDepth,
      shipRearWingExtensionLength: _shipConfig.shipConfig.shipRearWingExtensionLength,
      shipForwardThrusterMarginRatio: _shipConfig.shipConfig.forwardThrusterMarginRatio,
      shipForwardThrusterLength: _shipConfig.shipConfig.forwardThrusterLength,
      isStationary: false,
      scale: vec3.fromValues(1, 1, 1)
    };
    var dynamicsParams = {
      mass: _shipConfig.shipConfig.mass,
    ...
    var forceAppliers = [function (output, input) {
      return _this._applySpinSlowdown(output, input);
    }, _gamex.applyLinearDrag.bind(null, _config.physicsConfig), _gamex.applyAngularDrag.bind(null, _config.physicsConfig)];

    var _this = _possibleConstructorReturn(this, (ShipController.__proto__ || Object.getPrototypeOf(ShipController)).call(this, modelControllerParams, dynamicsParams, shapeParams, forceAppliers));

    _this._inputCtrl = inputCtrl;
    _this._healthCtrl = healthCtrl;
    _this._torpedoesCtrl = torpedoesCtrl;
    _this._forwardThrusterCtrl = null;
    _this._activeThrusters = null;
    _this._thrusterConfig = null;
    _this._isShipSpinningOutOfControl = false;

    _this._setUpThrusterModelControllers((0, _gamex.shallowCopy)(modelControllerParams));
    _this._setUpInputListeners();

    _this._patchModelController({
      updateChildren: function updateChildren(currentTime, deltaTime) {
        _this._forwardThrusterCtrl.updateSelfAndChildren(currentTime, deltaTime);
      },
      getIsReady: function getIsReady(superVersion) {
        return Promise.all([superVersion(), _this._forwardThrusterCtrl.getIsReady()]);
      }
    });
    return _this;
  }
  ...
  _createClass(ShipController, [{
    key: '_setUpThrusterModelControllers',
    value: function _setUpThrusterModelControllers(modelControllerParams) {
      var _this2 = this;

      modelControllerParams.getParentWorldTransform = function () {
        return _this2.modelCtrl._worldTransform;
      };

      var getIsForwardThrusterActive = function getIsForwardThrusterActive() {
        return _this2._activeThrusters.has('forward');
      };
      this._forwardThrusterCtrl = new _shipForwardThrusterController.ShipForwardThrusterController(modelControllerParams, getIsForwardThrusterActive);
    ...
    value: function _setUpInputListeners() {
      ...
      var throttledShootTorpedo = (0, _gamex.throttle)(function () {
        return _this3._shootTorpedo();
      }, _shipConfig.shipConfig._shootTorpedoThrottleDelay, true);
      this._inputCtrl.addKeyDownListener(_config.inputConfig.shipShootTorpedoKey, throttledShootTorpedo);
    }
    ...
    key: '_shootTorpedo',
    value: function _shootTorpedo() {
      var state = this.physicsJob.currentState;
      var sphere = this.physicsJob.collidable;

      var orientation = quat.clone(state.orientation);

      // Calculate the velocity.
      var velocity = vec3.fromValues(0, 0, -_shipConfig.shipConfig.torpedoSpeed);
      vec3.transformQuat(velocity, velocity, orientation);
      vec3.add(velocity, velocity, state.velocity);

      // Calculate the position.
      var shipSpeed = vec3.length(state.velocity);
      var shipDistanceNextFrame = shipSpeed * _config.physicsConfig.timeStepDuration * _torpedoDistanceMultiplier;
      var torpedoDistance = sphere.radius + _shipConfig.shipConfig.torpedoLength / 2 + shipDistanceNextFrame + _gamex.EPSILON;
      var position = vec3.fromValues(0, 0, -torpedoDistance);
      ...
      var torpedoParams = {
        ...
        length: _shipConfig.shipConfig.torpedoLength,
        width: _shipConfig.shipConfig.torpedoWidth,
        color: _shipConfig.shipConfig.torpedoColor.rgbVec,
        source: this
      };
    ...
    key: '_setUpThrusters',
    value: function _setUpThrusters() {
      var _this4 = this;
      ...
      this._thrusterConfig = {
        'forward': {
          key: _config.inputConfig.shipForwardThrusterKey,
          forceApplier: _applyForwardThrust
        },
        'stabilizer': {
          key: _config.inputConfig.shipStabilizerKey,
          forceApplier: _applyStabilizerThrust
        },
        'leftwardYaw': {
          key: _config.inputConfig.shipYawThrusterLeftKey,
          forceApplier: _applyYawThrust.bind(this, true)
        },
        'rightwardYaw': {
          key: _config.inputConfig.shipYawThrusterRightKey,
          forceApplier: _applyYawThrust.bind(this, false)
        },
        'upwardPitch': {
          key: _config.inputConfig.shipPitchThrusterUpKey,
          forceApplier: _applyPitchThrust.bind(this, true)
        },
        'downwardPitch': {
          key: _config.inputConfig.shipPitchThrusterDownKey,
          forceApplier: _applyPitchThrust.bind(this, false)
        }
      };

      Object.keys(this._thrusterConfig).forEach(function (thrusterName) {
        return _this4.registerThrusterInputListeners(thrusterName);
      });
    }
    ...
    key: 'registerThrusterInputListeners',
    value: function registerThrusterInputListeners(thrusterName) {
      var _this5 = this;

      var key = this._thrusterConfig[thrusterName].key;
      this._inputCtrl.addKeyDownListener(key, function () {
        return _this5._activateThruster(thrusterName);
      });
      this._inputCtrl.addKeyUpListener(key, function () {
        return _this5._deactivateThruster(thrusterName);
      });
    }
    ...
    key: '_activateThruster',
    value: function _activateThruster(thrusterName) {
    ...
    key: '_deactivateThruster',
    value: function _deactivateThruster(thrusterName) {
    ...
    key: '_applySpinSlowdown',
    value: function _applySpinSlowdown(output, input) {
      ...
      _applySpinSlowdown2(output, input);
    }
    ...
    key: '_isSomeShipRotationThrusterActive',
    value: function _isSomeShipRotationThrusterActive() {
      var _this6 = this;

      return !['leftwardYaw', 'rightwardYaw', 'upwardPitch', 'downwardPitch'].every(function (thruster) {
        return !_this6._activeThrusters.has(thruster);
      });
    }
    ...
    key: 'handleCollision',
    value: function handleCollision(collision) {
      var other = (0, _gamex.getOtherControllerFromCollision)(collision, this);
...
function _createInertiaTensor(shapeParams) {
  ...
  return (0, _gamex.createBoxInertiaTensor)(shapeParams.shipWidth, shapeParams.shipLength, shapeParams.shipDepth, _shipConfig.shipConfig.mass);
}
...
function _applyForwardThrust(output, input) {
  ...
  var acceleration = vec3.fromValues(0, 0, -_shipConfig.shipConfig.forwardThrustAccelerationMagnitude);
...
function _applyYawThrust(isLeftward, output, input) {
  ...
  var accelerationMagnitude = _shipConfig.shipConfig.yawAngularAcceleration * (isLeftward ? 1 : -1);
  var acceleration = vec3.fromValues(0, accelerationMagnitude, 0);
...
function _applyPitchThrust(isUpward, output, input) {
  // Create an angular acceleration vector.
  var accelerationMagnitude = _shipConfig.shipConfig.yawAngularAcceleration * (isUpward ? -1 : 1);
  var acceleration = vec3.fromValues(accelerationMagnitude, 0, 0);
...
function _applyMaxSpinLimiter(output, input) {
  var angularSpeed = vec3.length(input.state.angularVelocity);

  // If the ship isn't rotating too fast, then we don't need to slow it down.
  if (angularSpeed < _shipConfig.shipConfig.maxAngularSpeed) {
    return;
  }

  // Calculate how far over the max angular speed the ship is rotating.
  var angularSpeedDiff = angularSpeed - _shipConfig.shipConfig.maxAngularSpeed;
...
function _applySpeedSlowdown(output, input) {
  ...
  var slowDownMagnitude = speed > _shipConfig.shipConfig.slowdownAcceleration ? _shipConfig.shipConfig.slowdownAcceleration : speed;
 ...
 * @private
 */
function _applySpinSlowdown2(output, input) {
  var angularSpeed = vec3.length(input.state.angularVelocity);
  ...
  var slowDownMagnitude = angularSpeed > _shipConfig.shipConfig.angularSlowdownAcceleration ? _shipConfig.shipConfig.angularSlowdownAcceleration : angularSpeed;
  vec3.scale(acceleration, acceleration, -slowDownMagnitude);
...
function _applyWorldAngularAcceleration(output, input, acceleration) {
  ...
  var reverseRotation = quat.create();
...
function _getInitialMomentum() {
  return vec3.fromValues(0, 0, -_initialSpeed * _shipConfig.shipConfig.mass);
}
var _initialSpeed = 0.002;
...
var _torpedoDistanceMultiplier = 1.1;

exports.ShipController = ShipController;

},{"../../../config":97,"./ship-config":118,"./ship-forward-thruster-controller":120,"gamex":1}],120:[function(require,module,exports){
'use strict';
...
exports.ShipForwardThrusterController = undefined;
 ...
 */
var ShipForwardThrusterController = function (_StandardModelControl) {
  _inherits(ShipForwardThrusterController, _StandardModelControl);

  /**
   * @param {ModelControllerConfig} modelControllerParams
   * @param {Function.<boolean>} getIsThrusterActive
   */
  function ShipForwardThrusterController(modelControllerParams, getIsThrusterActive) {
    _classCallCheck(this, ShipForwardThrusterController);

    modelControllerParams.programWrapperId = _shipConfig.shipConfig.forwardThrusterShaderProgram;
    ...
    var _this = _possibleConstructorReturn(this, (ShipForwardThrusterController.__proto__ || Object.getPrototypeOf(ShipForwardThrusterController)).call(this, modelControllerParams, shapeParams));

    _this._color = vec4.fromValues(_shipConfig.shipConfig.forwardThrusterColor.rgbVec[0], _shipConfig.shipConfig.forwardThrusterColor.rgbVec[1], _shipConfig.shipConfig.forwardThrusterColor.rgbVec[2], _shipConfig.shipConfig.forwardThrusterColorAlpha);
    _this._getIsThrusterActive = getIsThrusterActive;
    return _this;
  }

  _createClass(ShipForwardThrusterController, [{
    key: 'update',
    value: function update(currentTime, deltaTime) {}
  }, {
    key: 'draw',
    value: function draw() {
      if (this._getIsThrusterActive()) {
        _programs.flatColorProgramWrapperConfig.draw.call(this);
      }
    }
  }]);

  return ShipForwardThrusterController;
}(_gamex.StandardModelController);

// Re-use methods that are common for the flat-color program.


ShipForwardThrusterController.prototype._setUpProgramVariablesConfig = _programs.flatColorProgramWrapperConfig.setUpProgramVariablesConfig;

exports.ShipForwardThrusterController = ShipForwardThrusterController;

},{"../../../programs":141,"./ship-config":118,"gamex":1}],121:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.shipForwardThrusterRenderableShapeFactory = undefined;

var _gamex = require('gamex');

var shipForwardThrusterRenderableShapeFactory = {
  shapeId: 'SHIP_FORWARD_THRUSTER',
   ...
   */
  getRenderableShape: function getRenderableShape(params) {
    var facelessVertexPositions = _calculateFacelessVertexPositions(params);
    var vertexPositions = _calculateVertexPositions(facelessVertexPositions);
    var vertexNormals = _calculateVertexNormals(vertexPositions);
    var textureCoordinates = _calculateTextureCoordinates(params);
   ...
   */
  getCacheId: function getCacheId(params) {
    return params.shapeId + ':' + params.shipWidth + ':' + params.shipLength + (':' + params.shipDepth + ':' + params.shipRearWingExtensionLength);
  }
};
...
var _faceVertexPositionIndices = [
// Top-left face
...
function _calculateFacelessVertexPositions(params) {
  var halfWidth = params.shipWidth / 2 * params.shipForwardThrusterMarginRatio;
  var halfDepth = params.shipDepth / 2 * params.shipForwardThrusterMarginRatio;
  var halfShipLength = params.shipLength / 2;
  var marginDepthOffset = params.shipRearWingExtensionLength * (1 - params.shipForwardThrusterMarginRatio);
  var epsilon = 0.001;
  var middleZ = halfShipLength - params.shipRearWingExtensionLength + epsilon;
  var outerZ = halfShipLength - marginDepthOffset + epsilon;
...
function _calculateVertexPositions(facelessVertexPositions) {
  ...
  for (var i = 0, count = _faceVertexPositionIndices.length; i < count; i++) {
    var currentFaceVertexPositionIndices = _faceVertexPositionIndices[i];
    for (var j = 0; j < 3; j++) {
      var currentPosition = facelessVertexPositions[currentFaceVertexPositionIndices[j]];
      for (var k = 0; k < 3; k++) {
...
function _calculateVertexNormals(vertexPositions) {
  return (0, _gamex.calculateOrthogonalVertexNormals)(vertexPositions);
}
...
exports.shipForwardThrusterRenderableShapeFactory = shipForwardThrusterRenderableShapeFactory;

},{"gamex":1}],122:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports._calculateFacelessVertexPositions = exports._faceVertexPositionIndices = exports.shipRenderableShapeFactory = undefined;

var _gamex = require('gamex');

var shipRenderableShapeFactory = {
  shapeId: 'SHIP',
 ...
 */
function _calculateFacelessVertexPositions(params) {
  var halfWidth = params.shipWidth / 2;
  var halfDepth = params.shipDepth / 2;
  var halfLength = params.shipLength / 2;
 ...
 */
function _calculateTextureCoordinates(params) {
  var shipRearWingExtensionRatio = params.shipRearWingExtensionLength / params.shipLength;
  var oneMinusShipRearWingExtensionRatio = 1 - shipRearWingExtensionRatio;

  return [
  // Top-left face
  .5, oneMinusShipRearWingExtensionRatio, 0, 1, .5, 0,
  // Top-right face
  1, 1, .5, oneMinusShipRearWingExtensionRatio, .5, 0,
  // Bottom-left face
  1, 1, .5, oneMinusShipRearWingExtensionRatio, .5, 0,
  // Bottom-right face
  .5, oneMinusShipRearWingExtensionRatio, 0, 1, .5, 0,
...
exports.shipRenderableShapeFactory = shipRenderableShapeFactory;
exports._faceVertexPositionIndices = _faceVertexPositionIndices;
exports._calculateFacelessVertexPositions = _calculateFacelessVertexPositions;
...
},{"gamex":1}],123:[function(require,module,exports){
'use strict';
...
var _starsConfig = require('./src/stars-config');

Object.keys(_starsConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _starsConfig[key];
    }
  });
});

var _starsController = require('./src/stars-controller');

Object.keys(_starsController).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _starsController[key];
    }
  });
});

},{"./src/stars-config":124,"./src/stars-controller":125}],124:[function(require,module,exports){
'use strict';
...
var starsFolderConfig = {
  label: 'Stars',
  config: starsConfig,
  isOpen: false
};

exports.starsConfig = starsConfig;
exports.starsFolderConfig = starsFolderConfig;
...
exports.StarsController = undefined;
...
var _starsConfig = require('./stars-config');
 ...
 */
var StarsController = function (_ModelController) {
  _inherits(StarsController, _ModelController);

  /**
   * @param {ModelControllerConfig} params
   * @param {Function.<vec3>} getCameraPosition
   */
  function StarsController(params, getCameraPosition) {
    _classCallCheck(this, StarsController);

    params.programWrapperId = _starsConfig.starsConfig.shaderProgram;

    var _this = _possibleConstructorReturn(this, (StarsController.__proto__ || Object.getPrototypeOf(StarsController)).call(this, params));

    _this._getCameraPosition = getCameraPosition;
    _this._programVariablesConfig = null;
    _this._positionsConfig = null;
    _this._sizesConfig = null;
    _this._colorsConfig = null;
    _this._mvMatrix = mat4.create();
    _this._starsPerChunk = new Map();
    return _this;
  }
  ...
  _createClass(StarsController, [{
    ...
    key: 'onChunkCreated',
    value: function onChunkCreated(chunk) {
      var starsForChunk = [];
      for (var i = 0; i < _starsConfig.starsConfig.starCountPerChunk; i++) {
        starsForChunk.push({
    ...
    key: 'onChunkDestroyed',
    value: function onChunkDestroyed(chunk) {
      this._starsPerChunk.delete(chunk.id);
    }
  }, {
    key: 'onAllChunksUpdated',
    value: function onAllChunksUpdated() {
      this._updateBuffers();
    }
    ...
    value: function _updateBuffers() {
      ...
      this._starsPerChunk.forEach(function (starsForChunk) {
        for (var i = 0, count = starsForChunk.length; i < count; i++, elementCount++) {
          var currentStar = starsForChunk[i];
      ...
      this._positionsConfig.buffer = (0, _gamex.createBufferFromData)(this._gl, flattenedPositions, this._gl.ARRAY_BUFFER, this._gl.DYNAMIC_DRAW);
      this._sizesConfig.buffer = (0, _gamex.createBufferFromData)(this._gl, flattenedSizes, this._gl.ARRAY_BUFFER, this._gl.DYNAMIC_DRAW);
      this._colorsConfig.buffer = (0, _gamex.createBufferFromData)(this._gl, flattenedColors, this._gl.ARRAY_BUFFER, this._gl.DYNAMIC_DRAW);
    ...
    value: function _getRandomPosition(bounds) {
      return vec3.fromValues((0, _gamex.randomFloatInRange)(bounds.minX, bounds.maxX), (0, _gamex.randomFloatInRange)(bounds.minY, bounds.maxY), (0, _gamex.randomFloatInRange)(bounds.minZ, bounds.maxZ));
    }
    ...
    value: function _getRandomSize() {
      return (0, _gamex.randomFloatInRange)(_starsConfig.starsConfig.minSize, _starsConfig.starsConfig.maxSize);
    }
    ...
    value: function _getRandomColor() {
      var hsl = {
        h: (0, _gamex.randomFloatInRange)(_starsConfig.starsConfig.minHue, _starsConfig.starsConfig.maxHue),
        s: (0, _gamex.randomFloatInRange)(_starsConfig.starsConfig.minSaturation, _starsConfig.starsConfig.maxSaturation),
        l: (0, _gamex.randomFloatInRange)(_starsConfig.starsConfig.minLightness, _starsConfig.starsConfig.maxLightness)
      };
      var rgb = (0, _gamex.hslToRgb)(hsl);
      return vec3.fromValues(rgb.r, rgb.g, rgb.b);
    }
  }]);

  return StarsController;
}(_gamex.ModelController);
...
},{"./stars-config":124,"gamex":1}],126:[function(require,module,exports){
'use strict';
...
var _torpedoController = require('./src/torpedo-controller');

Object.keys(_torpedoController).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _torpedoController[key];
    }
  });
});

var _torpedoesConfig = require('./src/torpedoes-config');

Object.keys(_torpedoesConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _torpedoesConfig[key];
    }
  });
});

var _torpedoesController = require('./src/torpedoes-controller');

Object.keys(_torpedoesController).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _torpedoesController[key];
    }
  });
});

},{"./src/torpedo-controller":127,"./src/torpedoes-config":128,"./src/torpedoes-controller":129}],127:[function(require,module,exports){
'use strict';
...
exports.TorpedoController = undefined;
...
var _torpedoesConfig = require('./torpedoes-config');
 ...
 */
var TorpedoController = function (_CollidablePhysicsMod) {
  _inherits(TorpedoController, _CollidablePhysicsMod);
   ...
   * @param {Function} onDestroyedCallback
   */
  function TorpedoController(modelControllerParams, torpedoParams, onDestroyedCallback) {
    _classCallCheck(this, TorpedoController);

    modelControllerParams.programWrapperId = _torpedoesConfig.torpedoesConfig.shaderProgram;

    var shapeParams = _createShapeParams(torpedoParams);
    var dynamicsParams = _createDynamicsParams(torpedoParams);
    var forceAppliers = [];

    var _this = _possibleConstructorReturn(this, (TorpedoController.__proto__ || Object.getPrototypeOf(TorpedoController)).call(this, modelControllerParams, dynamicsParams, shapeParams, forceAppliers));

    _this.source = torpedoParams.source;
    _this._onDestroyedCallback = onDestroyedCallback;

    _this.modelCtrl._color = vec4.fromValues(torpedoParams.color[0], torpedoParams.color[1], torpedoParams.color[2], _torpedoesConfig.torpedoesConfig.alpha);

    // Re-use some methods that are common for the flat-color program.
    _this._patchModelController({
      draw: _programs.flatColorProgramWrapperConfig.draw,
      _setUpProgramVariablesConfig: _programs.flatColorProgramWrapperConfig.setUpProgramVariablesConfig
    });
    return _this;
  }

  _createClass(TorpedoController, [{
    key: 'reset',
    value: function reset() {
      _get(TorpedoController.prototype.__proto__ || Object.getPrototypeOf(TorpedoController.prototype), 'reset', this).call(this);
...
function _createDynamicsParams(torpedoParams) {
  var momentum = vec3.create();
  vec3.scale(momentum, torpedoParams.velocity, _torpedoesConfig.torpedoesConfig.mass);
  return {
    mass: _torpedoesConfig.torpedoesConfig.mass,
...
exports.TorpedoController = TorpedoController;
...
},{"../../../programs":141,"./torpedoes-config":128,"gamex":1}],128:[function(require,module,exports){
'use strict';
...
function _updateRemovalSquaredDistance() {
  torpedoesConfig._removalSquaredDistance = torpedoesConfig.removalDistance * torpedoesConfig.removalDistance;
}

_updateRemovalSquaredDistance();

var torpedoesFolderConfig = {
  label: 'Torpedoes',
...
exports.torpedoesConfig = torpedoesConfig;
exports.torpedoesFolderConfig = torpedoesFolderConfig;
...
exports.TorpedoesController = undefined;
...
var _torpedoController = require('./torpedo-controller');
 ...
 */
var TorpedoesController = function (_ModelGroupController) {
  _inherits(TorpedoesController, _ModelGroupController);

  /**
   * @param {ModelGroupControllerConfig} modelControllerParams
   * @param {Function.<vec3>} getShipPosition
   */
  function TorpedoesController(modelControllerParams, getShipPosition) {
    _classCallCheck(this, TorpedoesController);

    var _this = _possibleConstructorReturn(this, (TorpedoesController.__proto__ || Object.getPrototypeOf(TorpedoesController)).call(this, modelControllerParams));

    _this._getShipPosition = getShipPosition;
    _this._throttledRemoveDistantTorpedoes = (0, _gamex.throttle)(function () {
      return _this._removeDistantTorpedoes();
    }, _torpedoesConfig.torpedoesConfig._removalCheckThrottleDelay);
    return _this;
  }

  _createClass(TorpedoesController, [{
    key: 'reset',
    value: function reset() {
      this.clearModelControllers();
    }
    ...
    key: 'createTorpedo',
    value: function createTorpedo(torpedoParams) {
      var _this2 = this;
      ...
      var torpedo = new _torpedoController.TorpedoController(modelControllerParams, torpedoParams, function (torpedo) {
        return _this2._onModelControllerDestroyed(torpedo);
      });
      this._startModelController(torpedo);
    }
  }, {
    key: '_removeDistantTorpedoes',
    value: function _removeDistantTorpedoes() {
      this._removeDistantModelControllers(this._getShipPosition(), _torpedoesConfig.torpedoesConfig._removalSquaredDistance);
    }
  ...
  return TorpedoesController;
}(_gamex.ModelGroupController);

exports.TorpedoesController = TorpedoesController;

},{"./torpedo-controller":127,"./torpedoes-config":128,"gamex":1}],130:[function(require,module,exports){
'use strict';
...
var _ufoController = require('./src/ufo-controller');

Object.keys(_ufoController).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _ufoController[key];
    }
  });
});

var _ufosConfig = require('./src/ufos-config');

Object.keys(_ufosConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _ufosConfig[key];
    }
  });
});

var _ufosController = require('./src/ufos-controller');

Object.keys(_ufosController).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _ufosController[key];
    }
  });
});

},{"./src/ufo-controller":131,"./src/ufos-config":132,"./src/ufos-controller":133}],131:[function(require,module,exports){
'use strict';
 ...
 */
var UfoController = function (_CollidablePhysicsMod) {
  _inherits(UfoController, _CollidablePhysicsMod);

  function UfoController() {
    _classCallCheck(this, UfoController);

    return _possibleConstructorReturn(this, (UfoController.__proto__ || Object.getPrototypeOf(UfoController)).apply(this, arguments));
  }

  return UfoController;
}(_gamex.CollidablePhysicsModelController);

},{"gamex":1}],132:[function(require,module,exports){
'use strict';
...
var ufosFolderConfig = {
  label: 'UFOs',
  config: ufosConfig,
  isOpen: false
};

exports.ufosConfig = ufosConfig;
exports.ufosFolderConfig = ufosFolderConfig;
 ...
 */
var UfosController = function (_ModelGroupController) {
  _inherits(UfosController, _ModelGroupController);

  function UfosController() {
    _classCallCheck(this, UfosController);

    return _possibleConstructorReturn(this, (UfosController.__proto__ || Object.getPrototypeOf(UfosController)).apply(this, arguments));
  }

  return UfosController;
}(_gamex.ModelGroupController);

},{"gamex":1}],134:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.blendProgramWrapperConfig = undefined;

var _gamex = require('gamex');

var blendProgramWrapperConfig = {}; /**
...
var _positionsConfig = null;
var _programVariablesConfig = null;

/** @param {WebGLRenderingContext} gl */
blendProgramWrapperConfig.initialize = function (gl) {
  _positionsConfig = (0, _gamex.create2DSquarePositionsConfig)(gl);
 ...
 */
blendProgramWrapperConfig.getProgramVariablesConfig = function (gl, inputFramebufferIds) {
  _programVariablesConfig.uniforms.uModelsSampler = _gamex.programWrapperStore.getTexture(inputFramebufferIds[0]);
  _programVariablesConfig.uniforms.uBrightnessSampler = _gamex.programWrapperStore.getTexture(inputFramebufferIds[1]);
  return _programVariablesConfig;
};

exports.blendProgramWrapperConfig = blendProgramWrapperConfig;

},{"gamex":1}],135:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.bloomProgramWrapperConfig = undefined;

var _gamex = require('gamex');

var _blendProgramConfig = require('./blend-program/blend-program-config');

var _blurHorizontalProgramConfig = require('./blur-horizontal-program/blur-horizontal-program-config');

var _blurVerticalProgramConfig = require('./blur-vertical-program/blur-vertical-program-config');

var _brightnessFilterProgramConfig = require('./brightness-filter-program/brightness-filter-program-config');

var _BLOOM_FRAMEBUFFER_1_ID = 'bloom-framebuffer-1'; /**
...
var _BLOOM_FRAMEBUFFER_2_ID = 'bloom-framebuffer-2';

var bloomProgramWrapperConfig = {};
...
bloomProgramWrapperConfig.childrenProgramConfigs = [_brightnessFilterProgramConfig.brightnessProgramWrapperConfig, _blurHorizontalProgramConfig.blurHorizontalProgramWrapperConfig, _blurVerticalProgramConfig.blurVerticalProgramWrapperConfig, _blendProgramConfig.blendProgramWrapperConfig];

bloomProgramWrapperConfig.childrenProgramsToDraw = [{
  programId: _brightnessFilterProgramConfig.brightnessProgramWrapperConfig.id,
  inputFramebufferIds: [_gamex.MODELS_FRAMEBUFFER_ID],
  outputFramebufferId: _BLOOM_FRAMEBUFFER_1_ID
}, {
  programId: _blurHorizontalProgramConfig.blurHorizontalProgramWrapperConfig.id,
  inputFramebufferIds: [_BLOOM_FRAMEBUFFER_1_ID],
  outputFramebufferId: _BLOOM_FRAMEBUFFER_2_ID
}, {
  programId: _blurVerticalProgramConfig.blurVerticalProgramWrapperConfig.id,
  ...
  programId: _blendProgramConfig.blendProgramWrapperConfig.id,
  inputFramebufferIds: [_gamex.MODELS_FRAMEBUFFER_ID, _BLOOM_FRAMEBUFFER_1_ID],
...
bloomProgramWrapperConfig.childrenFramebufferIds = [_BLOOM_FRAMEBUFFER_1_ID, _BLOOM_FRAMEBUFFER_2_ID];

/** @param {WebGLRenderingContext} gl */
bloomProgramWrapperConfig.webGLStateSetter = function (gl) {
  gl.disable(gl.DEPTH_TEST);
  gl.disable(gl.BLEND);
};

exports.bloomProgramWrapperConfig = bloomProgramWrapperConfig;

},{"./blend-program/blend-program-config":134,"./blur-horizontal-program/blur-horizontal-program-config":136,"./blur-vertical-program/blur-vertical-program-config":137,"./brightness-filter-program/brightness-filter-program-config":138,"gamex":1}],136:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.blurHorizontalProgramWrapperConfig = undefined;

var _gamex = require('gamex');

var blurHorizontalProgramWrapperConfig = {}; /**
...
/** @param {WebGLRenderingContext} gl */
blurHorizontalProgramWrapperConfig.initialize = function (gl) {
  _positionsConfig = (0, _gamex.create2DSquarePositionsConfig)(gl);
  _programVariablesConfig = {
    attributes: {
      aVertexPosition: _positionsConfig
    ...
    uniforms: {
      uSamplerSize: vec2.fromValues((0, _gamex.getViewportWidth)(), (0, _gamex.getViewportHeight)()),
      uSampler: null
    },
    mode: gl.TRIANGLES,
    elementCount: 6
 ...
 * @returns {ProgramVariablesConfig}
 */
blurHorizontalProgramWrapperConfig.getProgramVariablesConfig = function (gl, inputFramebufferIds) {
  _programVariablesConfig.uniforms.uSampler = _gamex.programWrapperStore.getTexture(inputFramebufferIds[0]);
  vec2.set(_programVariablesConfig.uniforms.uSamplerSize, (0, _gamex.getViewportWidth)(), (0, _gamex.getViewportHeight)());
  return _programVariablesConfig;
};

exports.blurHorizontalProgramWrapperConfig = blurHorizontalProgramWrapperConfig;

},{"gamex":1}],137:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.blurVerticalProgramWrapperConfig = undefined;

var _gamex = require('gamex');

var blurVerticalProgramWrapperConfig = {}; /**
                                            * This module defines the configuration for a WebGL program that processes an image to blur pixels
                                            * vertically.
                                            */
...
/** @param {WebGLRenderingContext} gl */
blurVerticalProgramWrapperConfig.initialize = function (gl) {
  _positionsConfig = (0, _gamex.create2DSquarePositionsConfig)(gl);
  _programVariablesConfig = {
    attributes: {
      aVertexPosition: _positionsConfig
 ...
 * @returns {ProgramVariablesConfig}
 */
blurVerticalProgramWrapperConfig.getProgramVariablesConfig = function (gl, inputFramebufferIds) {
  _programVariablesConfig.uniforms.uSampler = _gamex.programWrapperStore.getTexture(inputFramebufferIds[0]);
  vec2.set(_programVariablesConfig.uniforms.uSamplerSize, (0, _gamex.getViewportWidth)(), (0, _gamex.getViewportHeight)());
  return _programVariablesConfig;
};

exports.blurVerticalProgramWrapperConfig = blurVerticalProgramWrapperConfig;

},{"gamex":1}],138:[function(require,module,exports){
'use strict';

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.brightnessProgramWrapperConfig = undefined;

var _gamex = require('gamex');

var brightnessProgramWrapperConfig = {}; /**
...
/** @param {WebGLRenderingContext} gl */
brightnessProgramWrapperConfig.initialize = function (gl) {
  _positionsConfig = (0, _gamex.create2DSquarePositionsConfig)(gl);
  _programVariablesConfig = {
    attributes: {
      aVertexPosition: _positionsConfig
 ...
 * @returns {ProgramVariablesConfig}
 */
brightnessProgramWrapperConfig.getProgramVariablesConfig = function (gl, inputFramebufferIds) {
  _programVariablesConfig.uniforms.uSampler = _gamex.programWrapperStore.getTexture(inputFramebufferIds[0]);
  return _programVariablesConfig;
};

exports.brightnessProgramWrapperConfig = brightnessProgramWrapperConfig;

},{"gamex":1}],139:[function(require,module,exports){
'use strict';
...
var flatColorProgramWrapperConfig = {};
 ...
 * @param {WebGLRenderingContext} gl
 */
flatColorProgramWrapperConfig.webGLStateSetter = function (gl) {
...
exports.flatColorProgramWrapperConfig = flatColorProgramWrapperConfig;
...
Object.keys(_blendProgramConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _blendProgramConfig[key];
    }
  });
});
...
Object.keys(_blurHorizontalProgramConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _blurHorizontalProgramConfig[key];
    }
  });
});
...
Object.keys(_blurVerticalProgramConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _blurVerticalProgramConfig[key];
    }
  });
});
...
Object.keys(_brightnessFilterProgramConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _brightnessFilterProgramConfig[key];
    }
  });
});

var _flatColorProgramConfig = require('./flat-color-program/flat-color-program-config');

Object.keys(_flatColorProgramConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _flatColorProgramConfig[key];
    }
  });
});
...
var _starPointsProgramConfig = require('./star-points-program/star-points-program-config');

Object.keys(_starPointsProgramConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _starPointsProgramConfig[key];
    }
  });
});

var _bloomProgramConfig = require('./bloom-program-config');

Object.keys(_bloomProgramConfig).forEach(function (key) {
  if (key === "default" || key === "__esModule") return;
  Object.defineProperty(exports, key, {
    enumerable: true,
    get: function get() {
      return _bloomProgramConfig[key];
    }
  });
});

},{"./blend-program/blend-program-config":134,"./bloom-program-config":135,"./blur-horizontal-program/blur-horizontal-program-config":136,"./blur-vertical-program/blur-vertical-program-config":137,"./brightness-filter-program/brightness-filter-program-config":138,"./flat-color-program/flat-color-program-config":139,"./general-model-program/general-model-program-config":140,"./star-points-program/star-points-program-config":142}],142:[function(require,module,exports){
'use strict';
...
var starPointsProgramWrapperConfig = {};
 ...
 * @param {WebGLRenderingContext} gl
 */
starPointsProgramWrapperConfig.webGLStateSetter = function (gl) {
...
exports.starPointsProgramWrapperConfig = starPointsProgramWrapperConfig;
...
},{"./src/geometry":144}],144:[function(require,module,exports){
"use strict";
 ...
 */
function calculateInterceptVelocity(interceptVelocity, sourcePosition, sourceSpeed, targetPosition, targetVelocity) {
  // Calculate the direction from the source to the target.
  var directionToTarget = vec3.create();
  ...
  var targetVelocityParallelComponent = vec3.create();
  var parallelComponentScale = vec3.dot(targetVelocity, directionToTarget);
  vec3.scale(targetVelocityParallelComponent, directionToTarget, parallelComponentScale);

  // The orthogonal component.
  var targetVelocityOrthogonalComponent = vec3.create();
  vec3.subtract(targetVelocityOrthogonalComponent, targetVelocity, targetVelocityParallelComponent);

  var orthogonalSpeed = vec3.length(targetVelocityOrthogonalComponent);

  var isSourceFastEnoughToInterceptTarget = orthogonalSpeed < sourceSpeed;
  ...
  if (!isSourceFastEnoughToInterceptTarget) {
    ...
    var parallelSpeed = Math.sqrt(sourceSpeed * sourceSpeed - orthogonalSpeed * orthogonalSpeed);
    var sourceVelocityParallelComponent = vec3.create();
...
exports.calculateInterceptVelocity = calculateInterceptVelocity;

},{}]},{},[107])

//# sourceMappingURL=space-debris.js.map
